      ...
      container.innerHTML = `
        <div style="text-align: center; border-bottom: 2px solid #7c3aed; padding-bottom: 0.5in; margin-bottom: 0.75in;">
          ...
          <p style="color: #666; font-size: 14pt; margin: 0.3in 0; font-style: italic;">
            Raven Calder Journal Entry • ${journalEntry.metadata.sessionDate}
          </p>
        </div>
        ...
        <div style="margin-bottom: 0.75in;">
          <div style="font-size: 14pt; line-height: 2.0; text-align: justify; hyphens: auto;">
            ${journalEntry.narrative.split('\n').map((paragraph: string) =>
              paragraph.trim() ? `<p style="margin-bottom: 0.5in; text-indent: 0.5in;">${paragraph}</p>` : ''
            ).join('')}
          </div>
        </div>

        <div style="border-top: 1px solid #e5e7eb; padding-top: 0.5in; margin-top: 0.75in;">
          <h2 style="color: #7c3aed; font-size: 16pt; margin-bottom: 0.3in;">Session Analytics</h2>
          <div style="background: #f8fafc; padding: 0.4in; border-radius: 8px; border-left: 4px solid #7c3aed;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.3in;">
              <div>
                <p><strong>Total Interactions:</strong> ${journalEntry.metadata.totalInteractions}</p>
                <p><strong>Session Date:</strong> ${journalEntry.metadata.sessionDate}</p>
              </div>
              <div>
                <p><strong>Resonance Fidelity:</strong> ${journalEntry.metadata.resonanceFidelity}%</p>
                <p><strong>Session ID:</strong> ${data.sessionId.slice(-8)}</p>
              </div>
            </div>
            ${journalEntry.metadata.primaryPatterns.length > 0 ? `
              <div style="margin-top: 0.3in; border-top: 1px solid #e5e7eb; padding-top: 0.3in;">
                <p style="margin-bottom: 0.15in;"><strong>Primary Communication Patterns:</strong></p>
                <div style="display: flex; flex-wrap: wrap; gap: 0.1in;">
                  ${journalEntry.metadata.primaryPatterns.map((pattern: string) =>
                    `<span style="background: white; padding: 0.1in 0.2in; border-radius: 4px; border: 1px solid #d1d5db; font-size: 11pt;">${pattern}</span>`
                  ).join('')}
                </div>
              </div>
            ` : ''}
        ...
        <div style="margin-top: 0.75in; padding-top: 0.4in; border-top: 1px solid #e5e7eb; text-align: center; color: #666; font-size: 11pt;">
          <p>Generated by Raven Calder • Woven Web Application</p>
          <p style="font-style: italic; margin-top: 0.2in;">"Here's what resonated, here's what didn't, here's what pattern Raven is tentatively guessing — but you remain the validator."</p>
        </div>
      `;
