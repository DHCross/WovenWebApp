...
function transformFieldFileDaily(fieldFile = {}) {
  ...
  Object.entries(daily).forEach(([date, entry]) => {
    ...
    let magnitude = null;
    let directionalBias = null;

    if (meter) {
      if (typeof meter.magnitude === 'number') {
        magnitude = meter.magnitude;
      } else {
        const magX10 = toFiniteNumber(meter.mag_x10);
        magnitude = magX10 !== null ? magX10 / 10 : null;
      }

      if (typeof meter.directional_bias === 'number') {
        directionalBias = meter.directional_bias;
      } else {
        const biasX10 = toFiniteNumber(meter.bias_x10);
        directionalBias = biasX10 !== null ? biasX10 / 10 : null;
      }
    }
