 ...
 *
 * NOTE: This function now returns a placeholder. The actual metaphor generation
 * happens via LLM synthesis in lib/blueprint-narrator.ts using generateBlueprintMetaphor()
 *
 * The template-based approach has been retired to ensure unique, personalized metaphors.
 */
function generateBlueprintMetaphor(primary, secondary, shadow) {
  // Return structural placeholder - actual metaphor generation happens via LLM
  return `[Blueprint: ${primary.function}/${secondary.function}/${shadow.function} - metaphor pending LLM synthesis]`;
}
