...
export async function generateJournalPDFFast(journalEntry, sessionId, presentationStyle = 'technical') {
  try {
    ...
    yPos += 0.3;
    addSeparator();
    addText('📖 RELOCATION AND HOUSES', 12, 'bold', '#1e293b');
    yPos += 0.1;
    addText('Your planets never change signs or aspects — those are set the moment you were born. What does change when you move is how those planets are arranged into houses.', 9, 'normal', '#374151');
    yPos += 0.1;
    addText('The houses are tied to the local horizon and meridian (the Ascendant and Midheaven). When you relocate, those angles shift, and the houses are redrawn.', 9, 'normal', '#374151');
    yPos += 0.1;
    addText('A planet that was in your natal 11th house (friends, networks) might move into the 10th house (career, public life) when you live somewhere else.', 9, 'normal', '#374151');
    yPos += 0.1;
    addText('This doesn\'t erase your natal chart — it\'s like putting the same pattern into a new frame, showing how your inner blueprint plays out in a different environment.', 9, 'normal', '#374151');
    yPos += 0.1;
    addText('If your exact birth time isn\'t known, house information may be less reliable. In those cases, Raven will flag this with a House Uncertainty Notice.', 9, 'italic', '#6b7280');
