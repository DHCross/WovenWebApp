...
async function processMathbrain(event) {
  ...
  person_a: { details: { ...personAOriginal, name: personAOriginal.name || 'Subject' }, meta: deriveTimeMetaWithPolicy(personAOriginal, timePolicy) }
    ...
    if (wantBalanceMeter) {
      ...
      // Extract natal house cusps for transit-to-natal-house calculations
      if (natalResponse.data) {
        const houseCusps = extractHouseCusps(natalResponse.data);
        if (houseCusps) {
          result.person_a.chart.house_cusps = houseCusps;
          logger.debug('Extracted natal house cusps for Person A:', houseCusps.map(c => c.toFixed(2)));
        } else {
          logger.warn('Failed to extract house cusps from natal chart for Person A');
        }
      }
      ...
      // Extract natal house cusps for transit calculations
      if (natalResponse.data) {
        const houseCusps = extractHouseCusps(natalResponse.data);
        if (houseCusps) {
          result.person_a.chart.house_cusps = houseCusps;
          logger.debug('Extracted natal house cusps for Person A (main chart path):', houseCusps.map(c => c.toFixed(2)));
        }
      }
    ...
    if (wantSkyTransits && haveRange) {
      ...
      const { transitsByDate, retroFlagsByDate, provenanceByDate, chartAssets: transitChartAssets } = await getTransits(
        personA,
        {
          ...
          endDate: end,
          step: step,
          relocationMode: relocationSettings?.mode,
          locationLabel: relocationSettings?.locationLabel,
        },
        headers,
