    ...
    try {
      ...
      if (tl) {
        result.context.translocation = {
          ...
          tz: normalizeTimezone(tl.tz || transitA?.timezone || personA.timezone || 'UTC')
        };
      }
    } catch { /* ignore */ }
    ...
    try {
      if (relocationMode === 'A_local') {
        ...
        if (loc && typeof loc.lat === 'number' && typeof loc.lon === 'number') {
          ...
          result.provenance.relocation_coords = { lat: loc.lat, lon: loc.lon, tz: tz ? normalizeTimezone(tz) : null };
        }
        ...
        if (typeof transitA?.latitude === 'number' && typeof transitA?.longitude === 'number' && transitA?.timezone) {
          result.provenance.relocation_coords = {
            ...
            tz: normalizeTimezone(transitA.timezone)
          };
        }
      } else if (relocationMode === 'Custom') {
        const c = body.custom_location;
        if (c && typeof c.latitude === 'number' && typeof c.longitude === 'number') {
          const tz = transitA?.timezone || c.timezone || null;
          result.provenance.relocation_coords = {
            lat: c.latitude,
            lon: c.longitude,
            tz: tz ? normalizeTimezone(tz) : null
          };
        }
      }
    } catch { /* ignore */ }
