...
const { summarizeRelocation } = require('../relocation-runtime');
const {
...
// ============================================================================
// IMPORT REFACTORED MODULES VIA ORCHESTRATOR
// ============================================================================
// All extracted modules are re-exported through orchestrator.js for clean
// centralized imports. This keeps the monolith dependency graph clear.
const {
  validateSubject,
  validateSubjectLean,
...
const { composeWovenMapReport } = require('../../src/reporters/woven-map-composer.js');
...
// All validation, api-client, and seismograph-engine imports are now via orchestrator.js (above)
// Keeping this comment for reference during migration

// ... (rest of the file)

// The original validateSubjectLean is now replaced by the import.
// All core modules now imported via orchestrator.js for clean dependency management.
 ...
 */
// validateSubject is now imported from src/math-brain/validation.js
// normalizeSubjectData is now imported from src/math-brain/validation.js
...
function enrichDailyAspects(rawList, orbsProfile = 'wm-spec-2025-09') {
  ...
  for (const a of rawList) {
    const aspectName = (a.aspect || a.aspect_type || a.type || '').toLowerCase();
    const orb = typeof a.orbit === 'number' ? a.orbit : (typeof a.orb === 'number' ? a.orb : null);
    const p1 = a.p1_name || a.transiting_planet || a.point1 || a.first_planet;
    const p2 = a.p2_name || a.stationed_planet || a.point2 || a.second_planet;
...
// --- Transit Table Formatting & Seismograph Engine ---
// Both formatTransitTable and calculateSeismograph are now imported from src/math-brain/seismograph-engine.js
...
async function processMathbrain(event) {
  ...
  if (wantMock) {
    ...
    const translocation = body.translocation || {};
    const relocationApplied = translocation.applies === true && translocation.method === 'A_local';

    // Generate realistic seismograph data based on relocation
    // Hurricane Michael should produce strong negative (inward) directional bias
    const seismographData = relocationApplied
      ? { magnitude: 4.2, directional_bias: -4.1, volatility: 2.8 } // Strong compression from hurricane
      : { magnitude: 2.3, directional_bias: 3.0, volatility: 1.1 }; // Normal day
    ...
    const mock = {
      ...
      provenance: {
        ...
        engine: 'MathBrain',
        version: '0.0-dev',
        translocation_applied: relocationApplied,
        relocation_mode: relocationApplied ? translocation.method : 'none',
        houses_disclosure: relocationApplied
          ? `Houses recalculated: ${translocation.method}`
          : 'Houses not recalculated: natal'
      },
      context: {
      ...
      person_a: {
        ...
        chart: {
          ...
          aspects: [
            { p1_name: 'Sun', p2_name: 'Moon', aspect: 'square', orb: 2.5, _class: 'major' },
            { p1_name: 'Mars', p2_name: 'Jupiter', aspect: 'trine', orb: 1.8, _class: 'major' }
          ],
          transitsByDate: { [rangeStart]: [{ p1_name: 'Saturn', p2_name: 'Sun', aspect: 'square', orb: 0.5, _class: 'major' }] }
        },
        derived: { seismograph_summary: seismographData }
      },
  ...
  logger.info(`RapidAPI key check: ${rapidKey ? `present (${rapidKey.substring(0, 10)}...)` : 'missing'}`);
  if (!rapidKey) {
  ...
  const wantSynastry = modeToken === 'SYNASTRY' || modeToken === 'SYNASTRY_TRANSITS' || (body.snapshot_mode && hasPersonBInput);
  ...
  const activeTranslocation = relocationRequested ? translocationBlock : null;
  let relocationMode = 'none';
  if (relocationRequested) {
    relocationMode = normalizeRelocationMode(activeTranslocation?.method) || (fallbackModeToken && fallbackModeToken !== 'none' ? fallbackModeToken : 'Custom');
  }
  ...
  let relocationLabel = activeTranslocation?.current_location || (aLocal?.label ?? null);
  ...
  const transitSubjectProvided = Boolean(body.transit_subject || body.transit_subject_B || body.second_transit_subject);
  let tzAuthority = 'natal_record';
  if (isNowMode) tzAuthority = 'transit_now';
  if (relocationApplied) tzAuthority = 'relocation_block';
  else if (transitSubjectProvided && transitA?.timezone && !relocationCoords?.tz) tzAuthority = 'user_provided';
  ...
  // Best-effort: attach corpus-informed persona excerpt provenance if available on disk
  try {
    const fs = require('fs');
    ...
    if (fs.existsSync(provFile)) {
      try {
        ...
        if (rc.excerpt) {
          // Minimal sanitization: remove email/URL/phone-like numbers, collapse whitespace and truncate
          let tmp = String(rc.excerpt || '');
          tmp = tmp.replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, '[redacted]');
          tmp = tmp.replace(/https?:\/\/\S+/gi, '[redacted]');
          tmp = tmp.replace(/\+?\d[\d\-() ]{6,}\d/g, '[redacted]');
          tmp = tmp.split(/\r?\n/).map(l => l.trim()).filter(Boolean).join('\n');
          excerptText = tmp.slice(0, 1200);
    ...
    if (!excerptText && fs.existsSync(excerptFile)) {
      try {
        ...
        let tmp = String(raw || '').replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, '[redacted]');
        tmp = tmp.replace(/https?:\/\/\S+/gi, '[redacted]');
        tmp = tmp.replace(/\+?\d[\d\-() ]{6,}\d/g, '[redacted]');
        tmp = tmp.split(/\r?\n/).map(l => l.trim()).filter(Boolean).join('\n');
        excerptText = tmp.slice(0, 1200);
        personaSource.file = path.basename(excerptFile);
        personaSource.generated_at = personaSource.generated_at || new Date().toISOString();
      } catch (e) { }
    }
    if (excerptText) {
      result.provenance.persona_excerpt = result.provenance.persona_excerpt || excerptText;
      result.provenance.persona_excerpt_source = result.provenance.persona_excerpt_source || personaSource;
    }
  } catch (e) {
  ...
  try {
    const tl = relocationRequested ? activeTranslocation : translocationBlock;
    if (tl || relocationMode !== 'none') {
      const explicitMode = relocationMode !== 'none' ? relocationMode : null;
      const normalizedMethod = relocationRequested
        ? (explicitMode || normalizeRelocationMode(tl?.method) || null)
        : null;
      const ctxApplies = relocationRequested
        ? (normalizedMethod ? !['A_natal', 'B_natal', 'none'].includes(normalizedMethod) : false)
        : false;
      const ctxMethod = (() => {
        if (ctxApplies && normalizedMethod && normalizedMethod !== 'none') return normalizedMethod;
        if (normalizedMethod === 'none') return 'Natal';
        if (ctxApplies) return 'Custom';
        return 'Natal';
      })();
      const tzSource = ctxApplies
        ? (tl?.tz || relocationCoords?.tz || transitA?.timezone || personA.timezone || 'UTC')
        : (tl?.tz || null);
      const coordsBlock = (() => {
      ...
      const normalizedTz = tzSource ? normalizeTimezone(tzSource) : null;
      const ctx = {
        applies: ctxApplies,
        method: ctxMethod,
        house_system: houseSystem,
        tz: ctxApplies ? normalizedTz : null,
  ...
  try {
    ...
    // FIX: Only report relocation_mode if actually applied (coords were valid)
    // Previously, relocationMode was reported even when coords were missing
    result.provenance.relocation_mode = relocationApplied
      ? (relocationMode || 'Custom')
      : 'None';
  } catch { /* ignore */ }
  ...
  if (translocationApplies && wantBalanceMeter) {
    ...
    if (relocatedCoords && typeof relocatedCoords.latitude === 'number' && typeof relocatedCoords.longitude === 'number') {
      ...
      try {
        // Use dedicated V3 relocation endpoint for static chart
        const relocationResponse = await getRelocationChart(
          personA,
          {
            latitude: relocatedCoords.latitude,
            longitude: relocatedCoords.longitude,
            city: relocatedCoords.city
          },
          headers,
          pass
        );

        if (!relocationResponse || !relocationResponse.chart) {
          ...
          personAChartForSeismograph = relocationResponse.chart;

          // Store both charts in result for transparency
          result.person_a.chart_natal = personANatal.chart; // Blueprint
          result.person_a.chart_relocated = relocationResponse.chart; // Felt Weather
          result.person_a.chart = relocationResponse.chart; // Active chart = Felt Weather

          logger.info('[TRANSLOCATION] Successfully fetched Felt Weather chart (V3 Relocation)', {
            natal_asc: personANatal.chart?.angles?.Ascendant?.abs_pos,
            relocated_asc: relocationResponse.chart?.angles?.Ascendant?.abs_pos,
  ...
  if (wantSkyTransits && haveRange) {
    ...
    // Use V3 API for transits (supports date_range and strict coordinate priority)
    const { transitsByDate, retroFlagsByDate, provenanceByDate, chartAssets: transitChartAssets } = await getTransitsV3(
      subjectForTransits,
      {
        startDate: start,
        endDate: end,
  ...
  if ((dualNatalMode || (!relationshipMode && modeToken && modeToken.startsWith('NATAL') && modeToken.includes('TRANSITS')) && hasPersonB) && hasPersonB) {
    ...
    if (vBLeanPassive.isValid) {
      ...
      if (haveRange && !skipTransits && modeToken === 'DUAL_NATAL_TRANSITS') {
        try {
          // RELOCATION FIX FOR PERSON B (DUAL_NATAL_TRANSITS mode)
          // Determine if Person B should be relocated
          let subjectForTransitsB = personB;
          let activeHouseCuspsB = result.person_b?.chart?.house_cusps || null; // Default to natal/current chart

          const relocationModeLower = (relocationMode || '').toLowerCase();
          const bRelocationApplies = translocationApplies && (
            relocationModeLower === 'both_local' ||
            relocationModeLower === 'b_local' ||
            relocationModeLower === 'custom'
          );

          if (bRelocationApplies) {
            const relocatedCoords = translocationContext.coords || translocationContext.coordinates;
            if (relocatedCoords && typeof relocatedCoords.latitude === 'number' && typeof relocatedCoords.longitude === 'number') {
              // 1. Construct Relocated Subject
              const relocatedSubjectB = {
                ...personB,
                latitude: relocatedCoords.latitude,
                longitude: relocatedCoords.longitude,
                timezone: relocatedCoords.timezone || relocatedCoords.tz || personB.timezone,
                city: relocatedCoords.city || 'Relocated Location',
                nation: relocatedCoords.nation || personB.nation
              };

              // 2. Fetch Relocated Chart (Felt Weather for B)
              try {
                logger.info('[TRANSLOCATION] Fetching relocated chart for Person B in DUAL_NATAL_TRANSITS mode');
                const personBRelocated = await fetchNatalChartComplete(
                  relocatedSubjectB,
                  headers,
                  pass,
                  'person_b_relocated',
                  'dual_natal_transits_felt_weather_b'
                );

                if (personBRelocated) {
                  // Use Relocated Data
                  subjectForTransitsB = relocatedSubjectB;
                  activeHouseCuspsB = personBRelocated.chart.house_cusps;

                  // Store relocated chart in result for transparency
                  result.person_b.chart_natal = result.person_b.chart; // Blueprint
                  result.person_b.chart_relocated = personBRelocated.chart; // Felt Weather
                  result.person_b.chart = { ...result.person_b.chart, ...personBRelocated.chart }; // Merge to make active

                  logger.info('[TRANSLOCATION] Successfully fetched Felt Weather chart for Person B in DUAL_NATAL_TRANSITS', {
                    relocated_asc: personBRelocated.chart?.angles?.Ascendant?.abs_pos
                  });
                } else {
                  logger.warn('[TRANSLOCATION] Failed to fetch relocated chart for Person B - upstream returned null');
                }
              } catch (e) {
                logger.error('[TRANSLOCATION] Error fetching relocated chart for Person B in DUAL_NATAL_TRANSITS:', e.message);
              }
            }
          }
          ...
          const { transitsByDate: transitsByDateB, retroFlagsByDate: retroFlagsByDateB, provenanceByDate: provenanceByDateB } = await getTransits(
            ...
            headers,
            { ...pass, natalHouseCusps: activeHouseCuspsB }
          );
    ...
    if (modeToken === 'SYNASTRY_TRANSITS' && haveRange && !skipTransits) {
      ...
      if (bRelocationApplies) {
        ...
        if (relocatedCoords && typeof relocatedCoords.latitude === 'number' && typeof relocatedCoords.longitude === 'number') {
          ...
          // 2. Fetch Relocated Chart (Felt Weather for B)
          try {
            logger.info('[TRANSLOCATION] Fetching relocated chart for Person B (Felt Weather)');
            const personBRelocated = await fetchNatalChartComplete(
              relocatedSubjectB,
              headers,
              pass,
            ...
            if (personBRelocated) {
              ...
              logger.info('[TRANSLOCATION] Successfully fetched Felt Weather chart for Person B', {
                relocated_asc: personBRelocated.chart?.angles?.Ascendant?.abs_pos
              });
            } else {
              logger.warn('[TRANSLOCATION] Failed to fetch relocated chart for Person B - upstream returned null');
            ...
            logger.error('[TRANSLOCATION] Error fetching relocated chart for Person B:', e.message);
          }
        }
      }
...
// Expose shared helpers for downstream engines (seismograph, balance meter)
...
exports.getTransitsV3 = getTransitsV3;
