...
function resolveChartPreferences(options = {}) {
  return {
    theme: options.theme || process.env.ASTROLOGY_CHART_THEME || 'classic',
    language: options.language || 'EN',
    wheel_only: options.wheel_only ?? false,
    wheel_format: options.wheel_format || 'png',
  };
}

function stripGraphicsDeep(obj, options = {}) {
  return stripGraphicsDeepInternal(obj, options, []);
}

function stripGraphicsDeepInternal(obj, options, path) {
  if (!obj || typeof obj !== 'object') return obj;
  const kill = options.killKeys instanceof Set ? options.killKeys : GRAPHIC_KEYS;
  const preserve = options.preserve instanceof Set ? options.preserve : null;
  const collector = Array.isArray(options.collector) ? options.collector : null;

  if (Array.isArray(obj)) {
    return obj.map((item, index) => stripGraphicsDeepInternal(item, options, path.concat(index)));
  }

  const out = {};
  for (const [key, value] of Object.entries(obj)) {
    const nextPath = path.concat(key);
    if (kill.has(key) && (!preserve || !preserve.has(key))) {
      if (collector) {
        collector.push({ key, path: nextPath, value });
      }
      continue;
    }

    if (value && typeof value === 'object') {
      out[key] = stripGraphicsDeepInternal(value, options, nextPath);
    } else {
      out[key] = value;
    }
  }
  ...
  if (!transitsByDate || Object.keys(transitsByDate).length === 0) {
    ...
    const hasDailyIndices = Object.values(transitsByDate).some(day => {
      ...
      if (Array.isArray(day)) {
        return day.some(aspect => typeof aspect?.magnitude === 'number' && typeof aspect?.valence === 'number');
      }
      if (typeof day === 'object') {
        const balanceBlock = day.balance || day.indices;
        if (balanceBlock && typeof balanceBlock === 'object') {
          const hasMagnitude = typeof balanceBlock.magnitude === 'number';
          const hasValence = typeof balanceBlock.valence === 'number';
          if (hasMagnitude && hasValence) {
            return true;
          }
        }
        const aspects = resolveDayAspects(day);
        if (aspects.length) {
          return aspects.some(aspect => typeof aspect?.magnitude === 'number' && typeof aspect?.valence === 'number');
        }
      }
      return false;
    });
    ...
    if (!hasDailyIndices) {
      missing.push('magnitude', 'valence');
    }

    // Check for seismograph values
...
function evaluateMirrorReadiness(result) {
  ...
  const balanceReady = Boolean(
    wm?.balance_meter &&
    wm.balance_meter.magnitude?.value !== null &&
    wm.balance_meter.valence?.value !== null
  );
  if (!balanceReady) diagnostics.push('Balance meter values missing');
...
function calculateSeismograph(transitsByDate, retroFlagsByDate = {}, options = {}) {
  ...
  rawValenceSeries.push(valenceRaw);
    ...
    // Use seismograph's built-in directional_bias
    ...
    const magnitudeMethod = magnitudeMeta?.method || (rollingMagnitudes.length ? 'rolling_window_v3' : 'raw_direct_v3');
    ...
    const directionalScaling = scaleDirectionalBias(valenceRaw, {
      ...
      method: balanceVal != null ? 'balance_signed_v3' : 'raw_directional_v3'
    });
    ...
    const biasMethod = directionalScaling.meta?.method || (balanceVal != null ? 'balance_signed_v3' : 'raw_directional_v3');
    ...
    const dayEntry = {
      seismograph: {
        ...
        valence_bounded: biasSigned,
        valence: biasSigned,
        valence_label: biasInfo?.label || null,
        valence_code: biasInfo?.code || null,
        valence_raw_unbounded: +valenceRaw.toFixed(2),
        valence_calibrated: biasSigned,
        valence_range: directionalScaling.range,
        valence_version: calibrationMode,
        valence_polarity: biasPolarity,
        bias_signed: biasSigned,
        bias_abs: biasAbs,
        bias_polarity: biasPolarity,
        bias_label: biasInfo?.label || null,
        bias_code: biasInfo?.code || null,
        bias_motion: biasInfo?.motion || null,
        bias_direction: biasDirection,
        bias_method: biasMethod,
        bias_range: directionalScaling.range,
  ...
  const summaryDirectionalScaling = scaleDirectionalBias(rawAvg, {
    ...
    method: summaryValenceVersion === BALANCE_CALIBRATION_VERSION ? 'balance_signed_v3_summary' : 'raw_directional_v3_summary'
  });
  ...
  const biasSummaryMethod = summaryDirectionalScaling.meta?.method || (summaryValenceVersion === BALANCE_CALIBRATION_VERSION ? 'balance_signed_v3_summary' : 'raw_directional_v3_summary');
  ...
  const summary = {
    magnitude: magnitudeAvg,
    magnitude_label: magnitudeLabel,
    valence_bounded: biasRounded,
    valence: biasRounded,
    valence_label: biasSummaryInfo?.label || null,
    valence_code: biasSummaryInfo?.code || null,
    valence_emoji: biasSummaryInfo?.emoji || null,
    valence_polarity: biasSummaryPolarity,
    valence_raw_unbounded: rawAvgRounded,
    valence_version: summaryValenceVersion,
    valence_range: summaryDirectionalScaling.range,
    bias_signed: biasRounded,
    bias_abs: biasAbsRounded,
    bias_label: biasSummaryInfo?.label || null,
    bias_code: biasSummaryInfo?.code || null,
    bias_motion: biasSummaryInfo?.motion || null,
    bias_direction: biasSummaryDirection,
    bias_polarity: biasSummaryPolarity,
    bias_method: biasSummaryMethod,
    bias_range: summaryDirectionalScaling.range,
    volatility: volatilityAvg,
    volatility_label: volatilityInfo?.label || null,
...
function computeBidirectionalOverlays(synastryAspects, chartA, chartB, orbsProfile = 'wm-spec-2025-09') {
  ...
  // Compute separate Balance Meters for each direction
  const balanceAFromB = aggregate(aFromB);
  const balanceBFromA = aggregate(bFromA);

  return {
    a_from_b: {
      aspects: aFromB,
      balance_meter: balanceAFromB,
      description: `Person A experiences ${aFromB.length} contacts from Person B`
    },
    b_from_a: {
      aspects: bFromA,
      balance_meter: balanceBFromA,
...
/**
 * DEPRECATED: Old averaging approach - DO NOT USE
 * Compute relational Balance Meter for the bond itself (not just individuals)
 * @deprecated Use computeBidirectionalOverlays instead
 * @param {Array} synastryAspects - Cross-chart aspects
 * @param {Array} compositeAspects - Composite chart internal aspects
 * @param {Object} personATransits - Person A's transit data
 * @param {Object} personBTransits - Person B's transit data
 * @returns {Object} Relational balance meter data
 */
function computeRelationalBalanceMeter_DEPRECATED(synastryAspects, compositeAspects, personATransits, personBTransits) {
  // THIS IS THE INCORRECT "COLLAPSED BLOB" APPROACH
  // Kept for reference only - DO NOT USE
  ...
  for (const date of allDates) {
    ...
    dailyMetrics[date] = {
      ...
      person_a: { magnitude: magA, valence: valA, volatility: volA },
      person_b: { magnitude: magB, valence: valB, volatility: volB }
    };
  }

  // Calculate baseline relational valence from synastry
  const baselineValence = aspectCount > 0 ?
    Math.round((totalSupport - totalFriction) * 100) / 100 : 0;

  // Determine relational valence symbol
  let relationalValenceSymbol = '🌗'; // Default to mixed
  if (baselineValence > 1.0) relationalValenceSymbol = '🌞';
  else if (baselineValence < -1.0) relationalValenceSymbol = '🌑';
  ...
  return {
    baseline_valence: baselineValence,
    baseline_magnitude: Math.round(baselineMagnitude * 100) / 100,
    relational_valence: relationalValenceSymbol,
    ...
    climate_description: `Relational field showing ${relationalValenceSymbol} dynamic with ${baselineMagnitude.toFixed(1)} baseline intensity`,
    computation_note: 'Relational Balance Meter v1.0: Combines Person A + Person B daily transits with synastry baseline'
  };
}

/**
 * Transitional wrapper for relational Balance Meter calculations.
 *
 * The legacy implementation remains the only fully verified option at the moment,
 * but newer call sites expect a `computeRelationalBalanceMeter` helper. Until the
 * upgraded engine lands, delegate to the deprecated routine so reports continue
...
function computeRelationalBalanceMeter(
  ...
  return computeRelationalBalanceMeter_DEPRECATED(
    synastryAspects,
    compositeAspects,
    personATransits,
    personBTransits
...
function generateRelationalMirror(personA, personB, synastryAspects, composite, compositeTransits, orbsProfile = 'wm-spec-2025-09') {
  ...
  // Legacy function kept for compatibility - but marked deprecated
  const relationalBalanceMeter = computeRelationalBalanceMeter_DEPRECATED(
    synastryAspects,
    composite.aspects,
...
async function processMathbrain(event) {
  ...
  person_a: { details: { ...personAOriginal, name: personAOriginal.name || 'Subject' }, meta: deriveTimeMetaWithPolicy(personAOriginal, timePolicy) }
    ...
    let natalResponse;
    if (wantBalanceMeter) {
      // For Balance Meter runs, fetch natal chart for wheel graphics
      // (seismograph uses transits only for metrics, but UI needs chart wheels)
      natalResponse = await callNatal(
        API_ENDPOINTS.BIRTH_CHART,
        personA,
        headers,
        pass,
        'Birth chart (A) for Balance Meter'
      );
      ...
      appendChartAssets(result.person_a, chartAssets);
      // Extract chart wheel from top-level chart field
      if (natalResponse.chart) {
        ...
        appendChartAssets(result.person_a, wheelAssets);
      }
  ...
  const { transitsByDate, retroFlagsByDate, provenanceByDate, chartAssets: transitChartAssets } = await getTransits(personA, { startDate: start, endDate: end, step }, headers, pass);
      ...
      if (vBLeanPassive.isValid) {
        if (!result.person_b || !result.person_b.chart) {
          try {
            ...
            const { sanitized: chartDataB, assets: chartAssetsB } = sanitizeChartPayload(natalB.data || {}, {
              subject: 'person_b',
              chartType: 'natal',
              scope: 'natal_chart',
            });
            result.person_b = { details: personB, chart: chartDataB, aspects: Array.isArray(natalB.aspects) ? natalB.aspects : (chartDataB.aspects || []) };
            appendChartAssets(result.person_b, chartAssetsB);
            // Extract chart wheel SVG from top-level chart field
            if (natalB.chart) {
              const { assets: wheelAssetsB } = sanitizeChartPayload({ chart: natalB.chart }, {
                subject: 'person_b',
                chartType: 'natal',
                scope: 'natal_chart_wheel',
              });
              appendChartAssets(result.person_b, wheelAssetsB);
            }
          } catch (e) {
    ...
    if (wantBalanceMeter && haveRange) {
      ...
      if (relationshipMode && personB && !result.person_b?.chart?.transitsByDate) {
        ...
        if (!result.person_b || !result.person_b.chart) {
          try {
            ...
            const natalB = await callNatal(
              ...
              headers,
              pass,
              'Birth chart (B) for relational Balance Meter'
            );
            const { sanitized: chartDataB, assets: chartAssetsB } = sanitizeChartPayload(natalB.data || {}, {
              subject: 'person_b',
              chartType: 'natal',
        ...
        try {
          ...
          const { transitsByDate, retroFlagsByDate, provenanceByDate } = await getTransits(personB, { startDate: start, endDate: end, step }, headers, pass);
          const seismographData = calculateSeismograph(transitsByDate, retroFlagsByDate, {
            modeToken,
            isBalance: true,
            readiness: result.readiness,
...
function extractDayIndices(daySource) {
  // Extract s_plus, s_minus values from either compact day objects or raw aspect arrays
  const indices = { s_plus: 0, s_minus: 0 };
  ...
  const tryFromBlock = (block) => {
    ...
    applyScaled(plus, 's_plus');
    applyScaled(minus, 's_minus');
  };
