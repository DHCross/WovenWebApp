...
/**
 * v4 Relational Verdict (Label-Only, No Numeric SFD)
 * Derives cooperation/opposition hint from Directional Bias × Coherence.
 * 
 * @param {number} biasSigned5 - Directional bias on [−5, +5] scale
 * @param {number} coherence0to5 - Coherence on [0, 5] scale
 * @returns {string|null} - Label describing relational alignment
 */
function relationalVerdictLabel(biasSigned5, coherence0to5) {
  if (!Number.isFinite(biasSigned5) || !Number.isFinite(coherence0to5)) return null;
  
  // Scale coherence into [0..1] as "stability weight"
  const w = Math.max(0, Math.min(1, coherence0to5 / 5));
  
  // Weighted signal: negative = inward/contractive, positive = outward/expansive
  const signal = biasSigned5 * w;
  
  if (signal <= -2.5) return 'contractive alignment';
  if (signal < 2.5) return 'mixed vectors';
