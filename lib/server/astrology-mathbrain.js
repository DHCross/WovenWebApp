...
function extractHouseCusps(chartData) {
  if (!chartData || typeof chartData !== 'object') return { cusps: [], missing: [] };
  ...
  for (const houseName of houseNames) {
    ...
    if (houseData && typeof houseData.abs_pos === 'number') {
      ...
      // Missing house data - log and record
      logger.debug(`Missing house cusp data for ${houseName}`);
      missing.push(houseName);
    }
  }
...
async function fetchNatalChartComplete(subject, headers, pass, subjectLabel, contextLabel) {
  ...
  if (natalResponse.data) {
    const { cusps, missing } = extractHouseCusps(natalResponse.data);
    if (cusps.length > 0) {
      natalData.chart.house_cusps = cusps;
      logger.debug(`Extracted ${cusps.length} natal house cusps for ${subjectLabel}:`, cusps.map(c => c.toFixed(2)));
    }
    if (missing.length > 0) {
      logger.warn(`Missing house cusp data for ${subjectLabel}: ${missing.join(', ')}`);
      natalData.warnings = natalData.warnings || [];
      natalData.warnings.push({
        code: 'MISSING_HOUSE_CUSPS',
        message: `House cusp data is incomplete or unavailable for this chart. Please verify birth data or try again later. Missing: ${missing.join(', ')}`,
        details: { missing_houses: missing }
      });
    }
  }
...
async function computeComposite(A, B, pass = {}, H) {
  ...
  return { aspects: topAspects, raw: data };
    ...
    // Instead of throwing, return an object with an error message
    return {
      error: true,
      message: "Composite chart could not be generated at this time. Please check subject data and try again.",
