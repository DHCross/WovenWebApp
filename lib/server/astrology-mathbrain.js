      ...
      if (vBLeanPassive.isValid) {
        if (!result.person_b || !result.person_b.chart) {
          try {
            ...
            if (!personBNatal) {
              logger.warn('Failed to fetch natal chart for Person B in dual natal mode');
      ...
      if (vBLeanPassive2.isValid) {
        ...
        if (!result.person_b || !result.person_b.chart) {
          try {
            ...
            if (!personBNatal) {
              logger.warn('Failed to fetch natal chart for Person B in implicit dual mode - upstream returned null');
              result.person_b = { ...(result.person_b || {}), details: personB, error: 'Failed to compute Person B chart - upstream data unavailable' };
            } else {
              result.person_b = {
                details: personBNatal.details,
                chart: personBNatal.chart,
