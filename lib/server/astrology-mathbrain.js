...
function extractHouseCusps(chartData) {
  if (!chartData || typeof chartData !== 'object') {
    return { cusps: [], missing: [], error: 'Invalid chartData provided' };
  }
...
async function fetchNatalChartComplete(subject, headers, pass, subjectLabel, contextLabel) {
  ...
  if (natalResponse.data) {
    const { cusps: houseCusps, missing: missingCusps } = extractHouseCusps(natalResponse.data);
    if (houseCusps && houseCusps.length > 0) {
      ...
      if (missingCusps && missingCusps.length > 0) {
        logger.warn(`Partially extracted house cusps for ${subjectLabel}, missing: ${missingCusps.join(', ')}`);
        natalData.chart.missing_house_cusps = missingCusps;
      }
    } else {
      logger.warn(`Failed to extract any house cusps from natal chart for ${subjectLabel}`);
      if (missingCusps && missingCusps.length > 0) {
        natalData.chart.missing_house_cusps = missingCusps;
      }
    }
