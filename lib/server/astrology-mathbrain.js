...
function deriveDirectionalBias(rawVal, calibratedVal) {
  const raw = Number.isFinite(rawVal) ? rawVal : 0;
  const calibrated = Number.isFinite(calibratedVal) ? calibratedVal : null;

  if (calibrated === null) {
    return clamp(raw, -5, 5);
  }

  const magnitude = clamp(Math.abs(calibrated), 0, 5);
  let sign = raw === 0 ? 0 : Math.sign(raw);
  if (sign === 0 && magnitude > 0) {
    sign = Math.sign(calibrated) || 0;
  }

  if (sign === 0) return 0; // no directional evidence → neutral
  return clamp(sign * magnitude, -5, 5);
}
...
function calculateSeismograph(transitsByDate, retroFlagsByDate = {}, options = {}) {
  ...
  const enriched = enrichDailyAspects(rawDayAspects, orbsProfile);
    ...
    const biasSignedRaw = deriveDirectionalBias(valenceRaw, balanceVal);
    const biasInfo = classifyDirectionalBias(biasSignedRaw);
    const biasSigned = biasInfo?.value ?? +clamp(biasSignedRaw, -5, 5).toFixed(2);
    const biasAbs = +Math.abs(biasSigned).toFixed(2);
    const biasDirection = biasInfo?.direction || (biasSigned === 0 ? 'neutral' : (biasSigned > 0 ? 'outward' : 'inward'));
    const biasPolarity = biasSigned > 0 ? 'positive' : (biasSigned < 0 ? 'negative' : 'neutral');
    const biasMethod = balanceVal != null ? 'balance_signed_v1' : 'raw_clamp_v1';
  ...
  const biasSummaryInfo = classifyDirectionalBias(calibratedAvg);
  const biasSummaryValue = biasSummaryInfo?.value ?? +calibratedAvg.toFixed(2);
  const biasSummaryAbs = +Math.abs(biasSummaryValue).toFixed(2);
  const biasSummaryDirection = biasSummaryInfo?.direction || (biasSummaryValue === 0 ? 'neutral' : (biasSummaryValue > 0 ? 'outward' : 'inward'));
  const biasSummaryPolarity = biasSummaryValue > 0 ? 'positive' : (biasSummaryValue < 0 ? 'negative' : 'neutral');
  ...
  const biasSummaryMethod = summaryValenceVersion === BALANCE_CALIBRATION_VERSION ? 'balance_signed_v1' : 'raw_clamp_v1';
  const summary = {
    magnitude: +X.toFixed(2),
    magnitude_label: magnitudeInfo?.label || null,
    valence_bounded: biasSummaryValue,
    valence: biasSummaryValue,
    ...
    valence_range: [-5, 5],
    bias_signed: biasSummaryValue,
    bias_abs: biasSummaryAbs,
...
function computeBidirectionalOverlays(synastryAspects, chartA, chartB, orbsProfile = 'wm-spec-2025-09') {
  ...
  const balanceAFromB = computeSFD(aFromB, orbsProfile);
  const balanceBFromA = computeSFD(bFromA, orbsProfile);
...
function computeRelationalBalanceMeter_DEPRECATED(synastryAspects, compositeAspects, personATransits, personBTransits) {
  ...
  for (const date of allDates) {
    ...
    const sfdA = dayA.sfd_cont ?? 0;
    const sfdB = dayB.sfd_cont ?? 0;
    ...
    // SFD: Average (combined support-friction differential)
    const relationalSFD = (sfdA + sfdB) / 2;
    ...
    dailyMetrics[date] = {
      ...
      sfd_cont: Math.round(relationalSFD * 100) / 100,
      person_a: { magnitude: magA, valence: valA, volatility: volA, sfd: sfdA },
      person_b: { magnitude: magB, valence: valB, volatility: volB, sfd: sfdB }
    };
  }

  // Calculate baseline relational SFD from synastry
  const baselineSFD = aspectCount > 0 ?
    Math.round((totalSupport - totalFriction) * 100) / 100 : 0;

  // Determine relational valence symbol
  let relationalValence = '🌗'; // Default to mixed
  if (baselineSFD > 1.0) relationalValence = '🌞';
  else if (baselineSFD < -1.0) relationalValence = '🌑';
...
async function processMathbrain(event) {
  ...
  result.composite.transitsByDate = seismographData.daily;
    ...
    if (wantBalanceMeter && haveRange) {
      ...
      if (result.person_a?.chart?.transitsByDate) {
        ...
        if (relationshipMode && result.person_b?.chart?.transitsByDate) {
          ...
          balanceMeterReport.person_b_channel_summary = result.person_b.derived?.seismograph_summary || null;
          balanceMeterReport.person_b_daily_entries = result.person_b.chart.transitsByDate;
    ...
    if (relocationContext && wantBalanceMeter) {
