...
function assertSeismographInvariants(seismo) {
  ...
  const { magnitude: magRange, directional_bias: biasRange, coherence: cohRange } = spec.ranges;
  ...
  // Coherence check - optional in v5.0 (removed from core output)
  if (seismo.coherence !== undefined && seismo.coherence !== null) {
  ...
  const cohFinite = seismo.coherence !== undefined && seismo.coherence !== null ? Number.isFinite(seismo.coherence) : true; // true if not present
...
function assertBalanceMeterInvariants(result) {
  ...
  // Coherence inversion check - optional in v5.0 (coherence removed from core output)
  if (scaling.coherence_inversion !== undefined && scaling.coherence_inversion !== spec.coherence_inversion) {
    throw new BalanceMeterInvariantViolation(
  ...
  // Coherence check - optional in v5.0 (removed from core output)
  if (coherence && coherence.value !== undefined && coherence.value !== null) {
  ...
  if (!Number.isFinite(magnitude.value) || 
    ...
    throw new BalanceMeterInvariantViolation(
      ...
      { 
        ...
        coherence: coherence ? coherence.value : undefined
      }
    );
  }

  // Optional coherence finite check
  if (coherence && coherence.value !== undefined && coherence.value !== null && !Number.isFinite(coherence.value)) {
    throw new BalanceMeterInvariantViolation(
      'Non-finite coherence value detected',
