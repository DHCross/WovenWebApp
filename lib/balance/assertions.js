...
const spec = {
  ...
  ranges: {
    ...
    sfd: { min: RANGE_SFD[0], max: RANGE_SFD[1], null_display: 'n/a' },
  },
};
...
function assertBalanceMeterInvariants(result) {
  ...
  const { magnitude = {}, directional_bias: directionalBias = {}, coherence = {}, sfd = {} } = axes;
  ...
  const sfdValue = sfd.value;
  if (sfdValue !== null) {
    const numericSfd = Number(sfdValue);
    if (numericSfd < spec.ranges.sfd.min || numericSfd > spec.ranges.sfd.max) {
      throw new BalanceMeterInvariantViolation(
        `SFD out of range: ${numericSfd} not in [${spec.ranges.sfd.min}, ${spec.ranges.sfd.max}]`,
        { value: numericSfd, range: spec.ranges.sfd, normalized: sfd.normalized }
      );
    }
    if (!Number.isFinite(numericSfd)) {
      throw new BalanceMeterInvariantViolation(
        'Non-finite SFD value detected',
        { value: sfdValue }
      );
    }
  } else if (sfd.display !== spec.ranges.sfd.null_display) {
    throw new BalanceMeterInvariantViolation(
...
function assertSeismographInvariants(seismo) {
  ...
  if (sfd !== null) {
    const numericSfd = Number(sfd);
    if (numericSfd < -1 || numericSfd > 1) {
      throw new BalanceMeterInvariantViolation('SFD out of range', { value: numericSfd });
    }
    if (!Number.isFinite(numericSfd)) {
      throw new BalanceMeterInvariantViolation('Non-finite SFD value detected', { value: sfd });
    }
  }
