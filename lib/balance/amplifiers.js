...
function normalizeAmplifiedBias(amplifiedBias, context = {}) {
  if (!Number.isFinite(amplifiedBias)) {
    return 0;
  }

  const energy = Number.isFinite(context.energy) ? Math.abs(context.energy) : null;
  const stretch = Number.isFinite(context.stretch) && context.stretch > 0 ? context.stretch : 1.1;

  if (energy && energy > 0) {
    const divisor = Math.max(1, energy) * stretch;
    const normalized = amplifiedBias / divisor;
    if (normalized > 1) return 1;
    if (normalized < -1) return -1;
    return normalized;
  }

  let divisor = BIAS_DIVISOR;
  if (Number.isFinite(context.aspectCount) && context.aspectCount > 0) {
    divisor = Math.max(divisor, context.aspectCount * 0.5);
  }
