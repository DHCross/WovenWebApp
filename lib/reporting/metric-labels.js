...
const ORB_RESOLUTION_PROFILES = {
  ...
  STANDARD: {
    ...
    use_case: 'Daily climate calculations, SFD computation, general Balance Meter reports'
  },
...
function classifySfd(value) { // legacy
  const num = safeNumber(value, null);
  if (num == null) return null;
  const bounded = clamp(num, -5, 5);
  let disc = 0;
...
// Prevents artificial inflation of SFD by excluding out-of-orb “support”
function validateSfdOrbIntegrity(supportiveAspects, profileName = 'STANDARD') {
  const profile = getOrbProfile(profileName);
  const validated = supportiveAspects.filter(aspect => {
    const aspectType = aspect.type || 'major';
    const category = aspect.category || null;
    ...
    return aspect.orb <= maxOrb;
  });

  const filteredCount = supportiveAspects.length - validated.length;

  return {
    validated_aspects: validated,
    filtered_count: filteredCount,
    orb_integrity_maintained: filteredCount === 0,
    sfd_confidence: filteredCount === 0 ? 'high' : 'moderate',
    warning: filteredCount > 0 ? `${filteredCount} supportive aspect(s) filtered for orb violations` : null
  };
}

// Core: magnitude weighted by orb precision; SFD integrity guarded
function calculateBalanceMeterWithOrbIntegrity(aspectData, profileName = 'STANDARD') {
  ...
  // SFD integrity check (only supportive aspects considered here; friction handled elsewhere)
  const sfdData = validateSfdOrbIntegrity(
    included.filter(a => a.cooperation_type === 'supportive'),
    profileName
  );
